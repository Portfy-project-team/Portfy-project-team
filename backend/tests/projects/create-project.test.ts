...
describe("POST /api/projects", () => {
  ...
  it("CP-01 : create project with valid data", async () => {
    ...
    expect(res.status).toBe(201);
    expect(res.body.project.titre).toBe("Mon projet");
    expect(res.body.project.statusV).toBe("PENDING");
  });

  it("CP-02 : missing titre returns 400", async () => {
    const res = await request(app)
      .post("/api/projects")
      .set("Cookie", cookies)
  ...
  it("CP-03 : titre too short returns 400", async () => {
    const res = await request(app)
      .post("/api/projects")
      .set("Cookie", cookies)
      .send({ titre: "AB" });
  ...
  it("CP-04 : invalid type returns 400", async () => {
    const res = await request(app)
      .post("/api/projects")
      .set("Cookie", cookies)
      .send({ titre: "Mon projet", type: "INVALID" });
  ...
  it("CP-05 : no token returns 401", async () => {
    const res = await request(app)
      .post("/api/projects")
      .send({ titre: "Mon projet" });

    expect(res.status).toBe(401);
  });

  it("CP-06 : PROF cannot create project", async () => {
    const prof = await createAndLoginProf();
    const res = await request(app)
      .post("/api/projects")
      .set("Cookie", prof.cookies)
      .send({ titre: "Projet prof" });
