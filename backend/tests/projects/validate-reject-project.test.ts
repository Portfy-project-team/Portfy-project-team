...
describe("PUT /api/projects/:id/validate and reject", () => {
  ...
  let profCookies: string[] = [];
  let studentCookies: string[] = [];
  let projectIdForValidate: number;
  let projectIdForReject: number;
  ...
  beforeAll(async () => {
    ...
    // Créer et soumettre 2 projets
    const r1 = await request(app)
      .post("/api/projects").set("Cookie", studentCookies)
      .send({ titre: "Projet à valider" });
    projectIdForValidate = r1.body.project.id;
    await request(app).post(`/api/projects/${projectIdForValidate}/submit`).set("Cookie", studentCookies);

    const r2 = await request(app)
      .post("/api/projects").set("Cookie", studentCookies)
      .send({ titre: "Projet à rejeter" });
    projectIdForReject = r2.body.project.id;
    await request(app).post(`/api/projects/${projectIdForReject}/submit`).set("Cookie", studentCookies);
  });

  it("VR-01 : prof validate project", async () => {
    const res = await request(app)
      .put(`/api/projects/${projectIdForValidate}/validate`)
      .set("Cookie", profCookies)
      .send({ noteProf: "Excellent travail", score: 18 });
  ...
  it("VR-02 : prof reject project", async () => {
    const res = await request(app)
      .put(`/api/projects/${projectIdForReject}/reject`)
      .set("Cookie", profCookies)
      .send({ noteProf: "Incomplet", score: 8 });
  ...
  it("VR-03 : student cannot validate", async () => {
    const r = await request(app)
      .post("/api/projects").set("Cookie", studentCookies)
      .send({ titre: "Nouveau projet" });
    const pid = r.body.project.id;
    await request(app).post(`/api/projects/${pid}/submit`).set("Cookie", studentCookies);

    const res = await request(app)
      .put(`/api/projects/${pid}/validate`)
      .set("Cookie", studentCookies);

    expect(res.status).toBe(403);
  });

  it("VR-04 : no token returns 401", async () => {
    const res = await request(app)
      .put(`/api/projects/${projectIdForValidate}/validate`);
