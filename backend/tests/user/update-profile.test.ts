...
describe("PUT /api/user/me/profile", () => {
  ...
  it("UP-04 : invalid bio", async () => {
    ...
    // حيت bio ماكاينش ف schema غالبا غادي ترجع 500
...
//     expect(res.status).toBe(200);
//     expect(res.body.user.prof.nom).toBe("Prof");
//   });



//   it("UP-03 : update pro profile", async () => {
...
//     expect(res.status).toBe(200);
//     expect(res.body.user.professionnel.entreprise).toBe("Portfy");
//   });



//   it("UP-04 : unauthorized", async () => {
...
//     const hashedPassword = await bcrypt.hash("Secure123!!!", 12);
...
//     // // profCookies = profLogin.headers["set-cookie"];
//     //   profCookies = Array.isArray(profLogin.headers["set-cookie"])
//     //   ? profLogin.headers["set-cookie"]
//     //   : [];
...
//     // proCookies = proLogin.headers["set-cookie"];
//         proCookies = Array.isArray(proLogin.headers["set-cookie"])
//       ? proLogin.headers["set-cookie"]
//       : [];
//   });
