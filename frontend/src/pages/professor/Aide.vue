      ...
      <div class="doc-modal">
        ...
        <div class="doc-nav">
          <button
            ...
            :key="section.id"
            class="doc-nav-btn"
            :class="{ active: activeDoc === section.id }"
        ...
        <div class="doc-body">
          ...
          <div v-if="activeDoc === 'start'">
            ...
            <div class="doc-steps">
              <div class="doc-step"><span class="step-num">1</span><div><strong>Connexion</strong><p>Accédez à votre espace via vos identifiants institutionnels.</p></div></div>
              <div class="doc-step"><span class="step-num">2</span><div><strong>Dashboard</strong><p>Consultez vos statistiques : portfolios visités, recommandations publiées, commentaires reçus.</p></div></div>
              <div class="doc-step"><span class="step-num">3</span><div><strong>Portfolios</strong><p>Parcourez les portfolios de vos étudiants depuis la section dédiée.</p></div></div>
            </div>
          </div>
          ...
          <div v-if="activeDoc === 'comments'">
            ...
            <div class="doc-tip"><CheckCircle size="16" /><p><strong>Lire :</strong> Les commentaires non lus apparaissent avec un bandeau orange. Cliquez sur le cercle pour les marquer comme lus.</p></div>
            <div class="doc-tip"><CheckCircle size="16" /><p><strong>Répondre :</strong> Cliquez sur l'icône réponse pour envoyer un message directement à l'étudiant.</p></div>
            <div class="doc-tip"><CheckCircle size="16" /><p><strong>Filtrer :</strong> Utilisez les boutons "Tous", "Lus", "Non lus" pour trier les commentaires.</p></div>
          </div>
          ...
          <div v-if="activeDoc === 'settings'">
            ...
            <div class="doc-tip"><CheckCircle size="16" /><p><strong>Profil :</strong> Modifiez votre nom, email, téléphone et établissement. Ces informations sont affichées sur votre profil public.</p></div>
            <div class="doc-tip"><CheckCircle size="16" /><p><strong>Notifications :</strong> Activez ou désactivez les alertes email pour chaque type d'événement.</p></div>
            <div class="doc-tip"><CheckCircle size="16" /><p><strong>Sécurité :</strong> Changez votre mot de passe régulièrement. Minimum 8 caractères recommandé.</p></div>
            <div class="doc-tip warning"><AlertTriangle size="16" /><p>La suppression de compte est irréversible. Toutes vos données seront définitivement effacées.</p></div>
          </div>
        </div>
      </div>
    </div>
...
import { Search, BookOpen, MessageCircle, Mail, Phone, ChevronDown, Send, CheckCircle, X, Loader2, AlertTriangle, FileText, Star, MessageSquare, Settings } from 'lucide-vue-next'
...
const faqs = ref([
  {
    ...
    answer: 'Pour rédiger une recommandation, allez dans la section "Recommandations", cliquez sur "+ Ajouter une recommandation", remplissez le formulaire puis publiez.'
  },
  {
    question: 'Comment consulter les portfolios des étudiants ?',
    answer: 'Accédez à la section "Portfolios consultés" pour voir l’historique des portfolios.'
  },
  {
    question: 'Puis-je modifier une recommandation publiée ?',
    answer: 'Oui, vous pouvez modifier ou supprimer une recommandation depuis la section Recommandations.'
  },
  {
    question: 'Comment filtrer les portfolios ?',
    answer: 'Utilisez les filtres disponibles dans la page Portfolios consultés.'
  },
  {
    question: 'Comment gérer les notifications ?',
    answer: 'Allez dans Paramètres puis Notifications.'
  },
  {
    question: 'Où trouver mon historique d’activités ?',
    answer: 'Votre historique est visible sur le Dashboard.'
  },
  {
    question: 'Comment exporter mes données ?',
    answer: 'Allez dans Paramètres puis Exporter mes données.'
  },
  {
    question: 'Que faire si je rencontre un problème technique ?',
    answer: 'Contactez le support via le formulaire ou par email.'
  }
])
...
function toggleFaq(idx) {
  ...
  if (index > -1) expandedFaqs.value.splice(index, 1)
  else expandedFaqs.value.push(idx)
}
...
function copyPhone() {
  navigator.clipboard.writeText('+212 5 35 46 78 90').then(() => {
    ...
    showToast('Numéro copié dans le presse-papiers !')
...
function validateForm() {
  ...
  if (!contactForm.value.message.trim() || contactForm.value.message.length < 20) errors.message = 'Le message doit contenir au moins 20 caractères.'
...
.btn-submit { padding: 12px 24px; background: #e5b230; color: #fff; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: background .2s; display: flex; align-items: center; gap: 8px; }
.btn-submit:hover:not(.loading) { background: #e5b230; }
...
.close-btn { margin-left: auto; background: none; border: none; color: #64748b; cursor: pointer; padding: 4px; }
...
/* ─── Documentation Modal ─────────────────────────────── */
.doc-overlay { align-items: center; justify-content: center; }
.doc-modal {
  background: #fff;
  border-radius: 16px;
  width: 680px;
