  ...
  <div class="page-content">
    ...
    <div v-else class="reco-list">
      <div v-for="reco in recos" :key="reco.id" class="reco-card">
        <div class="reco-avatar" :style="{ background: reco.color }">{{ reco.initials }}</div>
        <div class="reco-body">
          <div class="reco-top">
            <div>
        ...
        <div class="reco-actions">
          <button class="action-btn" @click="openEditModal(reco)" title="Modifier"><Edit size="16" /></button>
          <button class="action-btn delete" @click="handleDelete(reco.id)" title="Supprimer"><Trash2 size="16" /></button>
        </div>
      </div>
      <div v-if="recos.length === 0" class="empty-state"><p>Aucune recommandation trouvée.</p></div>
    </div>
        ...
        <div class="modal-body">
          <template v-if="!editMode">
            ...
            <select v-model="form.portfolioId">
              ...
              <option v-for="p in visitedPortfolios" :key="p.id" :value="p.id">
...
async function fetchRecos() {
  loading.value = true
  try {
    const res = await recommandationsService.getAll({ filter: activeFilter.value, search: search.value })
...
onMounted(() => { fetchRecos(); fetchStats(); fetchVisitedPortfolios() })
...
async function handleSave() {
  modalError.value = ''
  if (!form.value.message.trim()) { modalError.value = 'Le message est requis.'; return }
  if (!editMode.value && !form.value.portfolioId) { modalError.value = 'Veuillez sélectionner un étudiant.'; return }
  saving.value = true
  try {
    if (editMode.value) {
      await recommandationsService.update(editId.value, { message: form.value.message })
    } else {
      await recommandationsService.create({ portfolioId: form.value.portfolioId, message: form.value.message })
    }
    closeModal(); fetchRecos(); fetchStats()
  } catch (err) {
...
<style scoped>
.page-content { padding: 24px 32px; background: #f8f9fb; min-height: 100%; font-family: 'Inter', sans-serif; }
...
.btn-primary { background: #e5b230; color: #fff; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: background .2s; }
.btn-primary:hover { background: #d4a020; }
.btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
.btn-secondary { background: transparent; border: 1px solid #cbd5e1; color: #475569; padding: 10px 20px; border-radius: 8px; cursor: pointer; font-weight: 500; }
.stats-row { display: flex; gap: 16px; margin-bottom: 24px; }
.stat-card { background: #fff; border-radius: 12px; padding: 20px; flex: 1; display: flex; align-items: center; gap: 16px; box-shadow: 0 1px 3px rgba(0,0,0,.06); }
...
.search-box { margin-left: auto; display: flex; align-items: center; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 7px 12px; gap: 8px; }
...
.reco-card { background: #fff; border-radius: 12px; padding: 18px 20px; display: flex; align-items: center; gap: 14px; box-shadow: 0 1px 3px rgba(0,0,0,.05); transition: box-shadow .2s; }
.reco-card:hover { box-shadow: 0 4px 12px rgba(0,0,0,.08); }
.reco-avatar { width: 42px; height: 42px; border-radius: 50%; color: #fff; font-weight: 700; display: flex; align-items: center; justify-content: center; font-size: 0.9rem; flex-shrink: 0; }
.reco-body { flex: 1; }
...
.action-btn { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 6px 10px; cursor: pointer; transition: background .2s; display: flex; align-items: center; justify-content: center; color: #64748b; }
...
.modal-header h3 { font-size: 1.1rem; font-weight: 700; margin: 0; color: #0f172a; }
...
.modal-body input, .modal-body textarea, .modal-body select { width: 100%; padding: 10px 12px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 0.875rem; color: #0f172a; outline: none; box-sizing: border-box; font-family: inherit; }
.modal-body textarea { resize: vertical; }
.modal-body input:focus, .modal-body textarea:focus, .modal-body select:focus { border-color: #e5b230; }
