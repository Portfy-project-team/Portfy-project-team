...
const isEditMode = computed(() => props.formationToEdit !== null)

watch(
  () => props.formationToEdit,
  (formation) => {
    if (formation) {
      form.title = formation.title || ''
      form.provider = formation.provider || ''
      form.type = formation.label || ''
      form.date = formation.date || ''
      form.progress = formation.progress !== undefined ? String(formation.progress) : ''
      form.tags = formation.tags ? formation.tags.join(', ') : ''
      form.certificateLink = formation.certificateLink || ''
    } else {
      form.title = ''
      form.provider = ''
      form.type = ''
...
function saveFormation(status) {
  if (!isFormValid.value) return

  const progressNumber = Number(form.progress)

  const formationData = {
    id: props.formationToEdit ? props.formationToEdit.id : Date.now(),
    title: form.title,
    provider: form.provider,
    status: props.formationToEdit ? props.formationToEdit.status : status,
    iconColor: props.formationToEdit ? props.formationToEdit.iconColor : 'cream',
    progress: progressNumber,
    progressColor: progressNumber === 100 ? 'green' : 'purple-orange',
    label: form.type,
