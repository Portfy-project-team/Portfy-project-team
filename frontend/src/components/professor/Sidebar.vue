  ...
  <aside :class="['sidebar', { closed: !isOpen }]">
    ...
    <nav class="sidebar-nav">
      ...
      <router-link to="/professor/commentaires" class="nav-item" active-class="active">
        ...
        <span v-if="isOpen && unreadComments > 0" class="badge">{{ unreadComments }}</span>
      </router-link>
...
async function fetchUnreadComments() {
  try {
    const res = await api.get('/professor/commentaires/stats')
    unreadComments.value = res.data.data.unread ?? 0
  } catch (err) {
    console.error('Erreur count commentaires:', err)
  }
}

onMounted(() => fetchUnreadComments())
